   ...
   return (
      <Link href={"#"} className={styles.item}>
         <div className={`${styles.item__imgBody} ${isView ? styles.item__imgBodyLine : ""}`}>
            <Image src={element.vrfGroup.image.node.sourceUrl} alt={element.vrfGroup.name} fill objectFit="contain" />
         </div>
         <div className={styles.item__title}>
            {isView ? "Инверторный кондиционер" + " " + element.vrfGroup.name : element.vrfGroup.name}
         </div>
      </Link>
   );
}
